	...
	protected static function check_requirements() {
		if ( ! self::is_php_compatible() ) {
			self::$errors[] = sprintf(
				...
				_x( '<strong>Store Credit for WooCommerce</strong> requires PHP %1$s or higher. You are using version %2$s', 'admin notice', 'woocommerce-store-credit' ),
			...
			self::$errors[] = sprintf(
				...
				_x( '<strong>Store Credit for WooCommerce</strong> requires WordPress %1$s or higher. You are using version %2$s', 'admin notice', 'woocommerce-store-credit' ),
				self::MINIMUM_WP_VERSION,
				get_bloginfo( 'version' )
			);
		} elseif ( ! self::is_wc_active() ) {
			self::$errors[] = _x( '<strong>Store Credit for WooCommerce</strong> requires WooCommerce to be activated to work.', 'admin notice', 'woocommerce-store-credit' );
		} elseif ( ! self::is_wc_compatible() ) {
			self::$errors[] = sprintf(
				/* translators: 1: Minimum WooCommerce version 2: Current WooCommerce version */
				_x( '<strong>Store Credit for WooCommerce</strong> requires WooCommerce %1$s or higher. You are using version %2$s', 'admin notice', 'woocommerce-store-credit' ),
...
WC_Store_Credit_Requirements::init();
