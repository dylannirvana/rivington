...
var elmId=self.getSpareElmId(type),elmName=self.getElmName(elmId),insert=self.getInsertPosition(parent,index);if(!$.isPlainObject(values)){values={}}
...
return{changed:hasChanged||hasDefaultValue,new:newShortcode,old:oldShortcode}};var cache=$usbcore.cache('_changeField');if(_skipSave!==!0&&instructions===!0&&!_currentData.isActiveRecoveryTask){var _shortcode=_updateShortcode(_currentData);if(_shortcode.changed&&instructions===!0){cache.set('shortcodeChanged',instructions)}
