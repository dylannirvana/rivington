...
dragData.lastInsert=$ush.clone(insert);if($usb.builder.isColumn(currentId)){dragData.place=self.getElmNode(currentId)}else{$usbcore.$remove(dragData.place);dragData.place=_document.createElement('div');dragData.place.className='usb_dropplace';var isHorizontalWrapper=($usb.builder.getElmName(targetContainerId)==='hwrapper');if($usb.builder.isRootElmContainer(targetContainerId)||isHorizontalWrapper||dragData.isParentTab){if(isHorizontalWrapper){var node;if($usbcore.$hasClass(targetContainer,'wrap')){node=($usb.builder.isMode('drag:add')&&children.length)?self.getElmNode(children[0]):dragData.target}
