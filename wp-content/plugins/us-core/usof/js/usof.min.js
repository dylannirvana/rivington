...
self.$head=$('head');self.$title=$('> .usof-form-row-title',self.$row);self.$typographyVars=$('#us-typography-vars');$('.usof-form-row-control:first [data-name]',self.$row).each(function(_,node){var $node=$(node),usofField=$node.data('usofField');if(usofField instanceof $usof.field){var name=$node.data('name');usofField.on('change',self._events.changePropField).on('syncResponsiveState',function(field,screenName){if(self.isLiveBuilder()&&$usb.find('preview')){$usb.preview.fieldSetResponsiveScreen(screenName)}
...
result[name]=value}});result=$ush.toString(result);self.$input.val(result).trigger('change');self.trigger('change',[result])},getValue:function(){return this.$input.val()||''},setValue:function(value,quiet){var self=this,data=$ush.toPlainObject(value);self.$input.val(''+value);this.hideMenu();if(!$.isEmptyObject(data)){if(self.hasDynamicValues){self.removeDynamicValue()}
