...
if ( ! function_exists( 'us_ajax_output_list_pagination' ) ) {
	...
	function us_ajax_output_list_pagination( $content ) {
		if (
			...
			if ( $nodes->count() ) {
				$us_ajax_list_index = isset( $_POST['us_ajax_list_index'] )
					? $_POST['us_ajax_list_index']
					: 0;

				$new_document = new DOMDocument;
				$new_document->appendChild( $new_document->importNode( $nodes->item( (int) $us_ajax_list_index ), TRUE ) );
